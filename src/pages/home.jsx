...
// --- নতুন আইকন ---
const WindDirectionIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 inline-block"><path d="M12 2L2 22l10-4 10 4z"></path></svg>;
const PrecipitationIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 inline-block"><path d="M12 15.562A8.002 8.002 0 004 9.373a8 8 0 1116 0c0 3.403-2.936 6.19-7 8.188l-1 1z"></path><path d="M12 14v2"></path></svg>;
const AirQualityIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 inline-block"><path d="M12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4zm0 6c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"></path><path d="M20.84 13.34c.1-.41.16-.83.16-1.26 0-3.31-2.69-6-6-6s-6 2.69-6 6c0 .43.06.85.16 1.26A4.002 4.002 0 004 17.5V20h16v-2.5c0-1.38-1.12-2.5-2.5-2.5h-.66z"></path></svg>;
const AlertIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 inline-block mr-2"><path d="M12 2L2 21h20L12 2z"></path><line x1="12" y1="8" x2="12" y2="14"></line><line x1="12" y1="18" x2="12.01" y2="18"></line></svg>;
...
// --- নতুন ফিচার: বাতাসের গুণমান (Air Quality) কার্ড ---
const AirQualityCard = ({ aqi }) => {
    const getAqiInfo = (value) => {
        if (value <= 50) return { level: 'Good', color: 'text-green-400', bgColor: 'bg-green-500/20' };
        if (value <= 100) return { level: 'Moderate', color: 'text-yellow-400', bgColor: 'bg-yellow-500/20' };
        if (value <= 150) return { level: 'Unhealthy for Sensitive Groups', color: 'text-orange-400', bgColor: 'bg-orange-500/20' };
        if (value <= 200) return { level: 'Unhealthy', color: 'text-red-500', bgColor: 'bg-red-500/20' };
        if (value <= 300) return { level: 'Very Unhealthy', color: 'text-purple-500', bgColor: 'bg-purple-500/20' };
        return { level: 'Hazardous', color: 'text-red-700', bgColor: 'bg-red-700/20' };
    };
    const aqiInfo = getAqiInfo(aqi);

    return (
        <div className="glass-card p-6 rounded-2xl">
            <h3 className="text-2xl font-bold mb-4 flex items-center"><AirQualityIcon /> <span className="ml-2">Air Quality Index</span></h3>
            <div className={`text-center p-4 rounded-lg ${aqiInfo.bgColor}`}>
                <p className={`font-bold text-5xl ${aqiInfo.color}`}>{aqi}</p>
                <p className={`mt-2 text-xl ${aqiInfo.color}`}>{aqiInfo.level}</p>
            </div>
            <p className="text-sm text-slate-300 mt-4 text-center">Based on US - EPA 2016 standards.</p>
        </div>
    );
};

// --- নতুন ফিচার: সতর্কতা (Weather Alerts) ---
const WeatherAlerts = ({ alerts }) => {
    if (alerts.length === 0) return null;
    return (
        <div className="glass-card p-6 rounded-2xl">
             <h3 className="text-2xl font-bold mb-4 flex items-center"><AlertIcon /> <span className="ml-2">Alerts & Advice</span></h3>
             <div className="space-y-3">
                {alerts.map((alert, index) => (
                    <div key={index} className="p-3 bg-yellow-500/20 rounded-lg text-yellow-300">
                        <p>{alert}</p>
                    </div>
                ))}
             </div>
...
const Home = () => {
    ...
    const fetchWeather = async ({ city = null, coords = null }) => {
        ...
        try {
            ...
            // --- API URL আপগ্রেড করা হয়েছে নতুন ডেটার জন্য ---
            const weather = await fetchAPI(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,wind_speed_10m,wind_direction_10m,us_aqi&hourly=temperature_2m,weather_code&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,precipitation_probability_max&timezone=auto`);
    ...
    // --- ডেটা প্রসেসিং এবং নতুন Helper ফাংশন ---
    const getWindDirection = (degrees) => {
        const directions = ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW'];
        return directions[Math.round(degrees / 45) % 8];
    };
    
    const { displayedWeather, hourlyForecast, weatherAlerts } = useMemo(() => {
        if (!weatherData) return { displayedWeather: null, hourlyForecast: [], weatherAlerts: [] };
        ...
        const dpWeather = {
            ...
            precipitationProbability: daily.precipitation_probability_max[selectedDayIndex], // নতুন ডেটা
            aqi: isToday ? current.us_aqi : null, // নতুন ডেটা
        ...
        if (isToday) {
            if (dpWeather.uv > 7) {
                alerts.push("High UV Index. It's recommended to use sunscreen.");
            }
            if (dpWeather.precipitationProbability > 50) {
                alerts.push(`High chance of rain (${dpWeather.precipitationProbability}%). Don't forget your umbrella!`);
            }
            if (dpWeather.aqi > 100) {
                 alerts.push("Air quality is moderate or worse. Consider limiting outdoor activities if you are sensitive.");
            }
        }

        return { displayedWeather: dpWeather, hourlyForecast: hForecast, weatherAlerts: alerts };
                        ...
                        <div className="animate-fade-in space-y-8">
                            ...
                            <div className="flex flex-col md:flex-row gap-8">
                                ...
                                <div className="hidden md:block w-full md:w-1/2 glass-card rounded-2xl p-6">
                                    ...
                                    {/* --- Today's Highlights (ডেস্কটপ) - নতুন তথ্যসহ --- */}
                                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-5 text-lg">
                                        {[
                                            ...
                                            { label: 'Wind', value: `${displayedWeather.wind} km/h`, Icon: WindIcon },
                                            { label: 'Direction', value: displayedWeather.windDirection, Icon: WindDirectionIcon },
                                            { label: 'Feels Like', value: `${displayedWeather.feelsLike}°`, Icon: FeelsLikeIcon },
                                            { label: 'Precipitation', value: `${displayedWeather.precipitationProbability}%`, Icon: PrecipitationIcon },
                                        ].filter(item => item.value !== null && !String(item.value).includes('null')).map(item => (
                            ...
                            {/* --- Today's Highlights (মোবাইল) - নতুন তথ্যসহ --- */}
                            ...
                            {/* --- নতুন সেকশন: সতর্কতা ও বাতাসের গুণমান --- */}
                            {selectedDayIndex === 0 && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {displayedWeather.aqi && <AirQualityCard aqi={displayedWeather.aqi} />}
