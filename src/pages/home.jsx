...
// --- নতুন ফিচার এর জন্য নতুন আইকন ---
const PrecipitationIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 inline-block"><path d="M20 16.58A5 5 0 0018 7h-1.26A8 8 0 104 15.25"></path><path d="M8 14.01l.01 0M12 16.01l.01 0M16 14.01l.01 0"></path></svg>;
const AirQualityIcon = () => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 inline-block"><path d="M3 12h1m8-9v1M12 21v1m9-9h-1M4.22 19.78l.71-.71M4.22 4.22l.71.71M19.78 19.78l-.71-.71M19.78 4.22l-.71.71"></path></svg>;
...
// --- নতুন ফিচার: AQI অনুযায়ী টেক্সট এবং কালার দেওয়ার জন্য Helper ফাংশন ---
...
const Home = () => {
    ...
    const fetchWeather = async ({ city = null, coords = null }) => {
        ...
        try {
            ...
            // --- নতুন ফিচার এর জন্য API URL আপগ্রেড করা হয়েছে ---
            const weather = await fetchAPI(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2d,relative_humidity_2m,apparent_temperature,weather_code,wind_speed_10m,precipitation_probability,us_aqi&hourly=temperature_2m,weather_code,precipitation_probability&daily=weather_code,temperature_2m_max,temperature_2m_min,sunrise,sunset,uv_index_max,precipitation_probability_max&timezone=auto`);
    ...
    const { displayedWeather, hourlyForecast } = useMemo(() => {
        ...
        const dpWeather = {
            ...
            temp: isToday ? Math.round(current.temperature_2d) : Math.round(daily.temperature_2m_max[selectedDayIndex]),
            ...
            // --- নতুন ডেটা ---
            precipitation: isToday ? current.precipitation_probability : daily.precipitation_probability_max[selectedDayIndex],
            aqi: isToday ? current.us_aqi : null,
        };
    ...
    // --- নতুন ডেটা ---
    const aqiInfo = displayedWeather && displayedWeather.aqi ? getAqiInfo(displayedWeather.aqi) : null;
                        ...
                        <div className="animate-fade-in space-y-8">
                            ...
                            <div className="flex flex-col md:flex-row gap-8">
                                ...
                                <div className="hidden md:block w-full md:w-1/2 glass-card rounded-2xl p-6">
                                     {/* --- Today's Highlights (ডেস্কটপ) - নতুন ফিচার সহ --- */}
                                    ...
                                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-5 text-lg">
                                        {[
                                            ...
                                            { label: 'Air Quality', value: aqiInfo ? aqiInfo.text : 'N/A', Icon: AirQualityIcon, aqi: true, color: aqiInfo?.color },
                                        ].filter(item => item.value && !item.value.includes('null')).map(item => (
                                            <div key={item.label} className="text-center p-3 rounded-lg bg-white/10 flex flex-col justify-center">
                                                <p className="font-light text-slate-300 text-base flex items-center justify-center gap-2">{item.Icon()} {item.label}</p>
                                                <p className={`font-bold text-2xl mt-1 ${item.color || ''}`}>{item.value}</p>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            {/* --- Today's Highlights (মোবাইল) - নতুন ফিচার সহ --- */}
                            <div className="block md:hidden glass-card rounded-2xl p-6">
                                ...
                                <div className="grid grid-cols-2 gap-5 text-lg">
                                    {[
                                        ...
                                        { label: 'Humidity', value: `${displayedWeather.humidity}%`, Icon: HumidityIcon },
                                        { label: 'Precipitation', value: `${displayedWeather.precipitation}%`, Icon: PrecipitationIcon },
                                        { label: 'Air Quality', value: aqiInfo ? aqiInfo.text : 'N/A', Icon: AirQualityIcon, color: aqiInfo?.color },
                                    ].filter(item => item.value && !item.value.includes('null')).map(item => (
                                         <div key={item.label} className="text-center p-2 rounded-lg bg-white/10">
                                            ...
                                            <p className={`font-bold text-xl mt-1 ${item.color || ''}`}>{item.value}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>

                            {/* --- Hourly Forecast - নতুন ফিচার সহ --- */}
                            {selectedDayIndex === 0 && (
                                            ...
                                            {hourlyForecast.map((hour, index) => {
                                                ...
                                                return (
                                                    <div key={index} className="p-4 rounded-xl text-center w-28 flex-shrink-0 bg-white/10">
                                                        ...
                                                        <p className="text-xs text-cyan-200 mt-1">{hour.precipitation}% Rain</p>
                                                    </div>
                                                );
                                            })}
                                        </div>
                                        ...
                                        {weatherData.daily.time.slice(0, 7).map((date, index) => {
                                            ...
                                            return (
                                                <div key={date} onClick={() => setSelectedDayIndex(index)} className={`p-4 rounded-xl cursor-pointer text-center w-32 flex-shrink-0 transition-all border-2 ${selectedDayIndex === index ? 'bg-white/30 border-white/50' : 'bg-white/10 border-transparent hover:bg-white/20'}`}>
                                                    ...
                                                    <p className="text-xs text-cyan-200 mt-1">{precipitation}% Rain</p>
                                                </div>
                                            );
                                        })}
                                    </div>
